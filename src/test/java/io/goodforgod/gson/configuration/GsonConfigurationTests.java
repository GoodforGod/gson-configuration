...
class GsonConfigurationTests extends Assertions {
  ...
  void configFailForNullFieldNamePolicy() {
    try {
      final GsonBuilder builder = new GsonConfiguration().setFieldNamingPolicy(null).builder();
  ...
  void configFailForNullLongPolicy() {
    try {
      final GsonBuilder builder =
          new GsonConfiguration().setLongSerializationPolicy(null).builder();
    } catch (IllegalArgumentException e) {
      assertFalse(e.getMessage().isEmpty());
    }
  }
  ...
  void configBuilderValidDateFormatISO() {
    ...
    final Timestamp timestamp =
        Timestamp.from(OffsetDateTime.ofInstant(Instant.EPOCH, ZoneId.of("UTC")).toInstant());
