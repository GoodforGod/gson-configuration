...
class TimestampPatternTests extends Assertions {
  ...
  private static final Timestamp VALUE_TIME =
      Timestamp.from(
          LocalDateTime.ofInstant(Instant.EPOCH, ZoneOffset.UTC).toInstant(ZoneOffset.UTC));
  private static final String VALUE = "1970-01-01T00:00:00.000Z";

  private final Gson adapter =
      new GsonBuilder().setDateFormat(DateTimeFormatters.ISO_DATE).create();
  ...
  void serializationIsValid() {
    ...
    final Pattern pattern =
        Pattern.compile("\"value\":\"1970-01-01T\\d\\d:00:00\\.000(Z|\\+\\d\\d:00)\"");
